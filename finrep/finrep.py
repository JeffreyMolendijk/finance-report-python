...
class finance:
    ...
    def __init__(self, savings: str, stocks: str, superannuation: str):
        ...
        stocks_range = stocks_range.merge(
            pd.concat(res), on=["date", "stock"], how="left"
        )
    ...
    def __validate_pandas(self):
        ...
        if (
            len(self.savings) == 2
            and len(self.superannuation) == 2
            and len(self.stocks) == 4
        ):
            return True
        return False
    ...
    def get_portfolio(self):
        ...
        Returns:
            _type_: pandas DataFrame
        """
        stockval = self.stocks[["date", "value", "stock"]].rename(
            columns={"value": "amount", "stock": "type"}
        )
